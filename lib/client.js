...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TEXT_FIELDS = [
			...
			["confirmAsk", "确认弹窗标题", "拖上去之后、真删之前的询问语（下面会列出会话名）"],
		...
		const CSS = [
			...
			`.dse-cfg-fold{flex:1;min-width:0;display:flex;align-items:center;gap:8px;padding:0;`,
			...
			`.dse-cfg-fold:focus-visible{outline:2px solid #7fd6a0;outline-offset:2px;border-radius:6px}`,
			`.dse-cfg-fold-caret{flex:none;width:11px;font-size:10px;color:var(--dsw-alias-label-tertiary,#9aa4bd)}`,
			`.dse-cfg-fold-title{font-size:13px;font-weight:600;letter-spacing:.02em}`,
			`.dse-cfg-fold-sum{flex:none;color:var(--dsw-alias-label-tertiary,#9aa4bd);font-size:12px}`,
			`.dse-cfg-fold-body{display:flex;flex-direction:column;gap:10px}`,
		...
		function Eater(props) {
			...
			const onDrop = (event) => {
				...
				if (cfg.confirm.enabled) {
					setState({ phase: "confirm", sessionId, title: title || sessionId });
					return;
				}
				...
				? jsxs("div", {
					...
					children: [
						...
						confirming ? jsx("div", {
							...
							role: "dialog",
							"aria-label": textOf("confirmAsk"),
							children: [
								jsx("div", {
									key: "ask",
									className: "dse-confirm-ask",
									children: textOf("confirmAsk")
								}),
								jsx("div", {
									key: "who",
				...
				children: [
					jsx("div", {
						...
						children: jsxs("button", {
							...
							onClick: onToggle,
							children: [
								jsx("span", { className: "dse-cfg-fold-caret", children: open ? "▾" : "▸" }),
				...
				children: [
					...
					jsx(Collapsible, {
						...
						children: [
							...
							eaten.length === 0
								? jsx("div", { key: "empty", className: "dse-cfg-hint", children: "还没有吃掉任何会话。" })
