...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TEXT_FIELDS = [
			...
			["restore", "撤销按钮", "侧边栏那个撤销按钮的字；留空只留 ↩ 图标（回执上的撤销会隐藏）"],
		...
		const CSS = [
			...
			`padding:4px 8px;border-radius:999px;border:1px dashed transparent;cursor:default;`,
			...
			// 药丸那一行：药丸占满剩余宽度，右侧跟一个撤销按钮
			`.dse-foot{display:flex;align-items:center;gap:6px;width:100%;min-width:0}`,
			`.dse-foot > .dse-pill{flex:1 1 auto;min-width:0}`,
			`.dse-undo{flex:none;display:inline-flex;align-items:center;justify-content:center;`,
			`box-sizing:border-box;height:26px;min-width:26px;padding:0 9px;border-radius:999px;`,
			`border:1px solid color-mix(in srgb,currentColor 35%,transparent);background:transparent;`,
			`color:var(--dsw-alias-label-secondary,#c3cadd);font-size:12px;line-height:16px;cursor:pointer;`,
			`transition:color .15s,border-color .15s,background .15s}`,
			`.dse-undo:hover{color:#7fd6a0;border-color:color-mix(in srgb,#7fd6a0 45%,transparent);`,
			`background:color-mix(in srgb,#7fd6a0 8%,transparent)}`,
			`.dse-undo:active{transform:translateY(1px)}`,
			...
			// 设置页
			`.dse-cfg{display:flex;flex-direction:column;gap:20px;max-width:640px;`,
			`color:var(--dsw-alias-label-primary,#e7ebf5);font-size:13px;line-height:20px}`,
			`.dse-cfg-hint{color:var(--dsw-alias-label-tertiary,#9aa4bd);font-size:12px;line-height:18px}`,
			`.dse-cfg-block{display:flex;flex-direction:column;gap:10px;padding:14px;border-radius:12px;`,
			`border:1px solid var(--dsw-alias-border-l1,#2c3350);background:var(--dsw-alias-bg-module-platform,rgba(255,255,255,.02))}`,
			`.dse-cfg-block > h3{margin:0;font-size:13px;font-weight:600;letter-spacing:.02em}`,
			...
			`.dse-cfg-seg{display:inline-flex;border:1px solid var(--dsw-alias-border-l2,#39415e);border-radius:9px;overflow:hidden}`,
			`.dse-cfg-seg button{border:0;background:transparent;color:inherit;font:inherit;`,
			`padding:5px 14px;cursor:pointer;opacity:.7}`,
			`.dse-cfg-seg button[data-active="true"]{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08));opacity:1}`,
			...
			`font:inherit;border-radius:9px;padding:5px 14px;cursor:pointer}`,
			...
			`.dse-cfg input[type="range"]{width:190px;accent-color:#7fd6a0}`,
			`.dse-cfg input[type="text"],.dse-cfg input[type="url"]{flex:1;min-width:0;box-sizing:border-box;`,
			`background:var(--dsw-alias-bg-base,transparent);border:1px solid var(--dsw-alias-border-l2,#39415e);`,
			`border-radius:9px;padding:5px 10px;color:inherit;font:inherit}`,
			`.dse-cfg-num{font-variant-numeric:tabular-nums;color:var(--dsw-alias-label-tertiary,#9aa4bd);min-width:52px;text-align:right}`,
			`.dse-cfg-field{display:flex;align-items:center;gap:10px}`,
			`.dse-cfg-field-label{flex:none;min-width:112px;color:var(--dsw-alias-label-secondary,#c3cadd)}`,
			`.dse-cfg-field input[type="text"]{flex:1;min-width:0;box-sizing:border-box;`,
			`background:var(--dsw-alias-bg-base,transparent);border:1px solid var(--dsw-alias-border-l2,#39415e);`,
			`border-radius:9px;padding:5px 10px;color:inherit;font:inherit}`,
			`.dse-cfg-blank{flex:none;font-size:11px;color:#ffb86b;border:1px solid rgba(255,184,107,.45);`,
			`border-radius:6px;padding:0 6px}`,
			`.dse-cfg-eaten{display:flex;align-items:center;gap:10px;min-width:0}`,
			...
			`background:radial-gradient(circle at 50% 40%,rgba(127,214,160,.07),transparent 70%)}`,
			`.dse-cfg-stage[data-drop="true"]{border-color:#7fd6a0;background:rgba(127,214,160,.1)}`,
			...
			`.dse-pet,.dse-mouth,.dse-pet::after,.dse-plate,.dse-toast{animation:none!important}}`
		].join("");
		...
		function Eater(props) {
			...
			const plateVisible = phase === "armed" || phase === "over" || phase === "eating" || phase === "eaten";
			...
			const plate = plateVisible && plateRect !== null
				? jsxs("div", {
					...
					style: {
						...
						height: `${plateRect.height}px`
					},
					...dropHandlers,
					children: [
						jsx(Pet, { key: "pet", size: cfg.size, nat, cfg, over, phase }),
						plateNote === "" ? null : jsx("div", {
							key: "note",
			...
			// 药丸行右侧的撤销按钮：撤最近吃掉的那一条。
			// 文案留空时只留 ↩ 图标，免得药丸行上出现一个没有意义的空白按钮。
			const latestEaten = eaten.length === 0 ? null : eaten[0];
			const undoText = textOf("restore");
			const undoLabel = undoText === "" ? "↩" : undoText;
			const undoHint = latestEaten === null ? "" : (
				`撤销最近吃掉的：${latestEaten.title}（${timeAgo(latestEaten.at)}）`
				+ (eaten.length > 1 ? `；台账里还有 ${eaten.length - 1} 条，见设置 → 会话喂鱼` : "")
			);

			return jsxs(react.Fragment, {
				children: [
					// 药丸这一行：图标 + 文案在左，撤销按钮靠右、与图标同一行对齐。
					jsxs("div", {
						key: "foot",
						className: "dse-foot",
						children: [
							...
							// 台账里有东西才出现 —— 没得撤销时不留一个死按钮。
							latestEaten === null ? null : jsx("button", {
								key: "undo",
								type: "button",
								className: "dse-undo",
								"data-role": "sidebar-undo",
								title: undoHint,
								onClick: () => void restore(latestEaten.sessionId),
								children: undoLabel
							}, "undo")
						]
					}),
		...
		function Toggle({ label, checked, onChange }) {
			return jsxs(Row, {
				...
				children: [
					...
					jsx("span", { className: "dse-cfg-hint", children: checked ? "开" : "关" })
				]
			});
		}
				...
				children: [
					...
					jsxs("div", { className: "dse-cfg-block", children: [
						jsxs("div", { className: "dse-cfg-row", children: [
							jsx("h3", { style: { flex: 1 }, children: "文案" }),
							jsx("button", {
								type: "button",
								className: "dse-cfg-btn",
					...
					jsxs("div", { className: "dse-cfg-block", children: [
						...
						eaten.length === 0
							? jsx("div", { className: "dse-cfg-hint", children: "还没有吃掉任何会话。" })
