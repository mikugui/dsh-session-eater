...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_TEXT = {
			...
			failed: "没吃下去",
			self: "别喂正在聊的这个会话",
			running: "会话还在跑，先停下来再喂",
			blank: "空白会话不用清理 —— DSH 需要时会立刻再建一个「新会话」",
			confirmAsk: "真的给我吃吗？",
			confirmAskTokens: "消耗了 {tokens} token，真的给我吃吗？",
			confirmYes: "删除",
			confirmNo: "取消"
		};
		...
		const TEXT_FIELDS = [
			...
			["running", "拒绝：正在跑的会话", "宿主还握着它的写句柄时的提示（留空则用宿主返回的原因）"],
		...
		const CSS = [
			...
			`.dse-confirm-ask{font-weight:600;line-height:17px}`,
			`.dse-confirm-actions{display:flex;gap:8px;justify-content:flex-end;margin-top:2px}`,
			...
			// 不能吃（空白会话 / 正在聊的那个）：整块变红 + 那行字变红，就地告诉用户为什么
			`.dse-plate[data-blocked="true"]{border-color:#e5484d;border-style:solid;`,
			`background:linear-gradient(to top,rgba(48,16,20,.94),rgba(22,12,16,.72));`,
			`box-shadow:0 0 0 4px rgba(229,72,77,.16)}`,
			`.dse-plate[data-blocked="true"] .dse-plate-note{color:#ff9aa0;font-weight:600}`,
			// 松手被拒：晃两下（只晃两下，不做成常驻抖动）
			`.dse-plate[data-phase="refused"]{animation:dse-refuse .48s ease-in-out 2}`,
			`@keyframes dse-refuse{0%,100%{transform:translateX(0)}20%{transform:translateX(-6px)}`,
			...
			// 确认阶段（等你点删除/取消的时候）：嘴**张着不动**（不是咀嚼）、脑袋慢慢点两下、
			// 腮红亮着 —— 一副"快给我吃"的渴望表情。刻意比悬停时慢，读起来是"等着"而不是"在啃"。
			`.dse-pet[data-eager="true"] .dse-mouth{opacity:1;transform:none;animation:none}`,
			`.dse-pet[data-eager="true"]{animation:dse-nod calc(var(--dse-chew,.54s) * 1.7) ease-in-out infinite}`,
			`.dse-pet[data-eager="true"][data-nod="false"]{animation:none}`,
			`.dse-pet[data-eager="true"]::after{content:"";position:absolute;inset:0;pointer-events:none;`,
			`background-image:radial-gradient(circle at 33% 71%,rgba(243,150,172,.3),transparent 13%),`,
			`radial-gradient(circle at 69% 71%,rgba(243,150,172,.3),transparent 13%);`,
			`opacity:.55}`,
		...
		function Pet(props) {
			...
			// 确认阶段（等你点「删除」/「取消」的时候）：嘴**张着不动**、脑袋慢慢点、
			// 腮红亮着 —— 一副"快给我吃"的渴望表情。跟悬停时那种咀嚼不一样：这里是静止张开的。
			const eager = phase === "confirm" && !forceOpen;
		...
		function Eater(props) {
			...
			const plateVisible = phase === "armed" || phase === "over" || phase === "confirm"
				|| phase === "refused" || phase === "eating" || phase === "eaten";
			...
			/**
			 * 就地拒绝一个"不能吃"的会话：投放区变红 + 写明原因 + 鱼闭嘴，1.8 秒后归位；
			 * 底部再补一条 toast（双通道 —— 眼睛盯着鱼身上也不会漏掉）。
			 *
			 * 「空白会话 / 正在聊的 / 正在跑的」三条理由**共用这一套**，反馈风格才一致
			 * （以前"正在跑的"走的是普通失败回执，看起来像出了错，其实也是"不能吃"）。
			 *
			 * @param {string} code - 原因码（blank / self / running）。
			 * @param {string} message - 给用户看的那句话。
			 * @param {string} [sessionId] - 会话 id。
			 * @param {string} [title] - 会话标题。
			 * @returns {void}
			 */
			const refuse = (code, message, sessionId = "", title = "") => {
				setState({ phase: "refused", blocked: { code, message }, sessionId, title });
				showToast({ kind: "error", message });
				// `state` 是模块级的活对象（setState 整体替换它），所以这里读到的是"此刻"的
				// phase —— 不会踩到闭包快照，归位一定触发。
				setTimeout(() => {
					if (state.phase === "refused") setState({ phase: "idle", blocked: null })
				}, 1800);
			};
			...
			const eat = useCallback(async (sessionId, title) => {
				...
				try {
					...
					if (!res.ok || payload === null || payload.ok !== true) {
						...
						// 保留宿主给的 code（例如 session-running）：下面要靠它区分
						// "不能吃"（就地拒绝）和"真失败"（失败回执）。
					...
					// 宿主那条 409「会话还在跑」也属于"不能吃"，跟前两条走同一套就地拒绝 UI
					// （以前它走的是普通失败回执，三条理由的反馈风格不一致）。
					if ((error && error.code) === "session-running") {
						refuse("running", textOf("running") || message, sessionId, title || sessionId);
						return;
					}
			 ...
			 * 这个会话能不能吃？不能就给个原因。
			 *
			 * 抽成一处、并且**在拖动过程中就调用** —— 以前只在松手时判断，结果用户把
			 * 「正在聊的这个会话」拖上来，鱼既没反应、提示又只出现在屏幕底部那条约 6 秒的
			 * toast 上（眼睛在左上角盯着鱼，很容易漏掉，就像"点了没反应"）。
			 * 现在悬停到鱼头上时投放区就变红并写出原因，松手还会摇头。
			 *
			 * @param {string} sessionId - 被拖的会话 id。
			 * @param {Element|null} row - 被拖的行（拿不到空白标记时兜底判断）。
			 * @returns {{code: string, message: string}} code 为 "" 表示可以吃。
			 */
			const refusalOf = (sessionId, row) => {
				// 1) 空白会话（无任何用户消息）优先拦下：喂了也会被 DSH 再建一个，
				//    "删了又回来"的迷惑感就是这么来的。
				const blank = isBlankSession(ctx, sessionId);
				if (blank === true || (blank === null && rowLooksBlank(row))) {
					return { code: "blank", message: textOf("blank") };
				}
				// 2) 正在聊的这个会话不能喂（宿主还握着它的写句柄）
				const current = currentSessionId(ctx);
				if (current !== "" && current === sessionId) {
					return { code: "self", message: textOf("self") };
				}
				return { code: "", message: "" };
			};
			...
			const onDragOver = (event) => {
				...
				const { sessionId } = readPayload(event);
				// 数据在 dragover 阶段才读得到（dragstart 是 capture 阶段，那时还是空的），
				// 所以"能不能吃"的判断放在这里，一悬停就给出醒目反馈。
				const refusal = /^session-/.test(sessionId) ? refusalOf(sessionId, dragRow.current) : { code: "", message: "" };
				const blocked = refusal.code === "" ? null : refusal;
				if (!over || (state.blocked?.code ?? null) !== (blocked?.code ?? null)) {
					setOver(true);
					setState({ phase: "over", blocked });
				}
			};
			...
			const onDrop = (event) => {
				...
				if (!/^session-/.test(sessionId)) {
					setState({ phase: "idle", blocked: null });
					return;
				}
				// 不能吃的（空白 / 正在聊的）：**就地**给出醒目反馈 —— 投放区变红、写明原因、
				// 鱼摇头一下，同时再补一条底部 toast。以前只有 toast，用户会以为"点了没反应"。
				const refusal = refusalOf(sessionId, row);
				if (refusal.code !== "") {
					refuse(refusal.code, refusal.message, sessionId, title || sessionId);
					return;
				}
			...
			// 被拒绝时投放区那行字用原因（"别喂正在聊的这个会话"），而不是"啊啊啊"
			const blocked = state.blocked ?? null;
			const plateNote = blocked !== null ? blocked.message
				: (over ? textOf("over") : textOf("dropHint"));
			...
			const plate = plateVisible && plateRect !== null
				? jsxs("div", {
					className: "dse-plate",
					"data-over": over && blocked === null ? "true" : "false",
					...
					...dropHandlers,
					children: [
						// 不能吃的时候鱼**闭着嘴**（over 传 false）：一副"这个我不吃"的样子，
						// 而不是张着嘴像要吃 —— 不然"没反应"和"在犹豫"分不清。
						jsx(Pet, { key: "pet", size: cfg.size, nat, cfg, over: over && blocked === null, phase }),
						...
						confirming ? jsx("div", {
							...
							children: [
								// 只留一句话：把会话名、轮数、缓存命中率那些都去掉了 ——
								// 弹窗越小越不打断，注意力只该在"删不删"上。
								jsx("div", {
									key: "ask",
