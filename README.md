...
- **点图标直达设置** —— 点侧栏那条药丸（或 Tab 到它 + 回车）= 打开「设置 → 会话喂鱼」，不用点两下
- **删除前确认** —— 松手先问一句「确定要删除这个会话吗？」，**可以在设置里关掉**
- **两条撤销路** —— 回执上的「撤销」（刷新即消失）+ 设置里 12 条持久台账（跨刷新保留）
- **15 段界面文案可自定义**，留空＝不显示；形象、身材、嘴位、动画也都能调
- 零运行时依赖、无构建步骤
...
4. 松手 —— **先在会话列表里问一句**，并列出会话名与它消耗的 token，两个按钮：

   ![删除前确认](docs/confirm.png)

   ```
   你确定要吃掉这个消耗了 1.2M token 的会话吗？
   想吃大白饭的大肥鱼
   62 轮 · 缓存命中 97%
                                    [取消] [删除]
   ```
   ...
   > 读不到时（比如空白会话）会退回不带数字的那条询问语，而不是硬写个"未知"。
   > 总量 = 未缓存输入 + 输出 + 缓存读 + 缓存写；缓存读通常占大头，所以副信息行里标了命中占比。
...
- **文案** / **最近吃掉**：这两个区块很长，**默认为折叠状态**，点标题展开（标题右侧会给
  `15 段` / `2 条` 这样的摘要，不展开也知道里面有多少东西）。收起时正文**根本不渲染**，
...
| 确认弹窗标题 | 读不到用量时的那句询问（下面自动列出会话名） | `你确定要吃掉这个会话吗？` |
| 确认弹窗标题（带用量） | `{tokens}` 换成实际用量；留空则退回上一条 | `你确定要吃掉这个消耗了 {tokens} token 的会话吗？` |
